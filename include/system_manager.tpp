...
std::shared_ptr<T>
SystemManager::register_system(int priority, Args&&... args) {
...
inline void
SystemManager::entity_signature_changed(Entity entity, Signature signature) {
    for (const auto& [type_name, system] : systems) {
        const auto& system_signature = signatures[type_name];
        // If system signature matches new signature (no change), keep/add
        // entity Otherwise, new signature doesn't match this system, ensure
        // entity is remove if exists
